...
export default function Tables() {
  ...
  const [data, setData] = useState<ProjectData>({ totalRecords: 0, data: [] });
    ...
    (user: any, columnKey: any) => {
      ...
      switch (columnKey) {
        ...
        case "defaultLanguage":
          return (
            languages.find((lang) => lang.key === cellValue)?.value ?? cellValue
          );
        case "availableLanguages":
          return cellValue
            .map((lang: string) => languages.find((l) => l.key === lang)?.value)
            .join(", ");
        case "CreatedAt":
        case "UpdatedAt":
          ...
          return (
            <Dropdown>
              ...
              <DropdownMenu variant="flat">
                <DropdownItem
                  key="edit"
                  onClick={() => router.push(`/projects/edit/${user.id}`)}
                >
                  Изменить
                </DropdownItem>
                <DropdownItem
                  ...
                  key="delete"
                  onClick={() =>
                    backend.deleteProject(user.id).then(() => router.refresh())
                  }
                >
                  Удалить
                </DropdownItem>
  ...
  return (
    ...
    >
      <TableHeader>
        ...
        <TableColumn key="defaultLAnguage">Язык по умолчанию</TableColumn>
        <TableColumn key="availableLanguage">Доступные языки</TableColumn>
